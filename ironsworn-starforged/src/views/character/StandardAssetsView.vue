...
const getAssetById = (asset: any) => {
    const assets = starforged['Asset Types'].find((x: IAsset) => x.Name === asset.category).Assets
    if (!assets) {
        return Effect.fail(new Error(`No assets found for category: ${asset.category}`))
    }

    const selectedAsset = assets.find((x: IAsset) => x.$id === asset.id_dataforged)

    if (!selectedAsset) {
        return Effect.fail(new Error(`No asset found for: ${asset.id_dataforged}`))
    }
