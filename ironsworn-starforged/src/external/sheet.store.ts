...
import { metaStore } from '@/external/meta.store';
 ...
 * */
export const useStarforgedSheetStore = defineStore(
  'starforgedSheetStore',
  () => {
    ...
    const dehydrateStore = () => {
      ...
      storeKeys.forEach((key) => {
        if (key === 'meta') {
          const { name, bio, gmNotes, avatar } = Effect.runSync(
    ...
    const setPermissions = (owned: boolean, gm: boolean) => {
      const permissions = metaStore.get().context.permissions
      permissions.isOwner = owned;
      permissions.isGM = gm;
    };
