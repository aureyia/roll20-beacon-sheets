...
const singleRoll = (seed: string) => ({
    ...
    results: {
        ...
        dice: [fuzzList[Effect.runSync(number_between(seed, 'dice-0', 0, 13))]],
        expression: '1d6',
        rolls: [
            {
                sides: 6,
                dice: 1,
                results: [fuzzList[Effect.runSync(number_between(seed, 'dice-0', 0, 13))]],
            },
        ],
    },
})
...
const resultObjects = (seed: string) => {
    const numberOfRolls = Effect.runSync(number_between(seed, 'numberOfRolls', 0, 10))
