...
export const stats = (seed: string) => ({
  edge: Effect.runSync(numberBetween(seed, 'edge', -5, 5)),
  heart: Effect.runSync(numberBetween(seed, 'heart', -5, 5)),
  iron: Effect.runSync(numberBetween(seed, 'iron', -5, 5)),
  shadow: Effect.runSync(numberBetween(seed, 'shadow', -5, 5)),
  wits: Effect.runSync(numberBetween(seed, 'wits', -5, 5)),
});

export const resources = (seed: string) => ({
  health: Effect.runSync(numberBetween(seed, 'health', -5, 10)),
  spirit: Effect.runSync(numberBetween(seed, 'spirit', -5, 10)),
  supply: Effect.runSync(numberBetween(seed, 'supply', -5, 10)),
  xp: Effect.runSync(numberBetween(seed, 'xp', -10, 10)),
});
...
const createTask = (seed: string, salt: number) => ({
  ...
  progress: Effect.runSync(numberBetween(seed, `${salt}-progress`, -20, 60)),
  ...
  countdown: Effect.runSync(
    numberBetween(seed, `${salt}-countdown`, -10, 10),
  ) as Countdown,
});
