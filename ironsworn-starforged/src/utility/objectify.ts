...
export const arrayToObject = (array: { _id: string; [x: string]: any }[]) => {
  ...
  if (!isValidArray) Effect.fail(new Error('Tried to objectify an array, but not every item had ids'));
...
export const objectToArray = (object: Record<string, any>) => Effect.sync(() => {
  if (!object) return [];
  const newArray: any[] = [];
