...
const abilities = props.storedAsset.abilities.filter((x: Ability) => x !== null);
...
<template>
  <Card class="asset-card rounded-xlg mb-8 w-[43%] border-0 drop-shadow">
    <CardHeader class="rounded-t-lg bg-card-input text-xl font-bold drop-shadow">{{
      dataforgedAsset.Name
    }}</CardHeader>
    <CardContent class="asset-body">
      ...
      <div class="abilities mt-4">
        <div class="mb-2 flex" v-for="(ability, index) in dataforgedAsset.Abilities">
    ...
    <CardFooter v-if="storedAsset.meter">
      <ToggleGroup>
        <ToggleGroupItem v-for="value in Array.from(storedAsset.meter)" :value="value.toString()">{{
          value
        }}</ToggleGroupItem>
      </ToggleGroup>
    </CardFooter>
