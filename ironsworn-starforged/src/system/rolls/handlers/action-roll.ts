        ...
        return {
            roll: (actor, modifier, momentum, rollName) =>
                Effect.gen(function* () {
                    ...
                    const output = yield* Effect.retry(
                        dispatch.roll(encodedDice),
                        {
                            times: 3,
                        }
                    )
                    ...
                    const { totalActionScore, dieNegated } =
                        yield* actionScore.calculate(
                            decodedDice,
                            modifier,
                            momentum,
                            null
                    ...
                    const { challengeDie1, challengeDie2, actionDie } =
                        yield* Effect.all({
                            challengeDie1: getDieByLabel(
                                decodedDice,
                                'Challenge Die: 1'
                            ),
                            challengeDie2: getDieByLabel(
                                decodedDice,
                                'Challenge Die: 2'
                            ),
